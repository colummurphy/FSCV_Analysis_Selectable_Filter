...
fscvdata=load([hgui.PathName, hgui.FileName]); 

if isfield(fscvdata,'recordedData')
    is16ChannelData = true;
           
    app.setAnodalLimit(fscvdata.anodalLimit);
    app.setCathodalLimit(fscvdata.cathodalLimit);

    app.setSamplesPerScan(fscvdata.samplesPerScan);
    app.setScanFrequency(fscvdata.scanFrequency);       
    app.setSampleFrequency(fscvdata.sampleFrequency);
...
if ~isequal(hgui.cname,cursess)
    ...
    if strcmp(hgui.subject,'cleo')
        % CM - new file
        [parameters,csc_map,eventcodes]=CM_getparams(app, is16ChannelData, ...
            hgui.cname,'cleo',ncschannels);
    else
        % CM - new file
        [parameters,csc_map,eventcodes]=CM_getparams(app, is16ChannelData, ...
            'patrabipolar','default',ncschannels,'sessnum',hgui.sessionnum);
    end
end
...
CM_getplotsettings(app, plotParam.filtlfp,plotParam.cscNames,...
